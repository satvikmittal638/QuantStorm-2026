...
"""
qs_bot.py -- Divided Oracle entry (v8 Adaptive Game-Theoretic Engine).

Core Strategic Pillars:

1. BAYESIAN OPPONENT PROFILING & ADAPTIVE HONESTY FILTERING.
   Maintains a real-time belief state tracking:
   - Quote feasibility (|midpoint| <= 4r, cross-round drift <= 4*delta_r).
   - Forensic cross-validation against FORESIGHT leaks.
   - Opponent bidding propensity (passive sniper vs aggressive forcer).
   Quote trust and noise parameters scale continuously with posterior honesty.

2. INVENTORY-SKEWED MARKET MAKING (TE AS INVENTORY).
   In real-world algorithmic market making, quotes skew with inventory risk.
   When our Tactical Energy (TE) is in deficit relative to the opponent,
   we tighten quote widths to harvest Maker obligation safely and eliminate
   tail straddle miss variance.

3. DYNAMIC FIRST-PRICE AUCTION ENGINE (EQUILIBRIUM BIDDING).
   Replaces static bidding with dynamic budget-aware game-theoretic sizing:
   - Snipes with 1 TE when opponent is broke (te_theirs == 0).
   - Adjusts shading dynamically based on opponent bidding history.
   - Caps bids at (te_theirs + 1) to prevent overpaying.

4. ASYMMETRIC SUBSTITUTE CONVEXITY ENGINE (BACHELIER OPTION PRICING).
   Evaluates capped downside at -2.0 ticks using the Bachelier normal option
   formula E[max(X, -2.0)], capturing the volatility bonus sigma * phi(z).
   Defensively prices E[min(X, +2.0)] when the opponent holds SUBSTITUTE.

5. EXACT PARITY-LATTICE STRADDLE & TAKER LAST-TURN FORCED FILL.
   S is always EVEN. Quotes align to even parity. On Turn 6, Taker forces
   at max((bid + ask)//2, ask - floor//2) + shift, extracting structural rent
   with TRICK_ROOM (+3) and STEALTH_ROCK (+2).
"""
...
class Bot:
    ...
    def reset(self, seat: int, config, seed: int) -> None:
        ...
        self.p_honest = 0.80   # Prior probability opponent quotes truthfully
        self.p_passive = 0.20  # Prior probability opponent bids minimally
        self.p_forcer = 0.30   # Prior probability opponent stalls to Turn 6
    ...
    def _refresh(self, obs) -> None:
        ...
        if obs.foresight and self.reads and obs.round in self.reads:
            ...
            if abs(q_mid - f_sum) > 2.5:
                self.p_honest = 0.0  # Confirmed adversarial quote manipulation
            else:
    ...
    def _their_k(self, obs):
        ...
        # Dynamically scale variance and inclusion by Bayesian honesty
    ...
    def quote(self, obs):
        ...
        for w in range(floor, cap + 1):
            ...
            # Inventory skew: if in TE deficit, penalize wide spread uncertainty
