...
#: Edge (ticks) an early trade must clear before we take it instead of
#: riding to the final turn.
#:
#: Against wide-quoting opponents this wants to be enormous -- the last-turn
#: option is worth more than almost any early fill, and refusing to trade
#: before it measured +2.4 ticks/deal against the three reference bots. That
#: result does not survive contact with opponents who quote tightly and
#: price well: against those, riding past a real edge just hands them the
#: fill they wanted. Swept on the full panel, the answer is small.
RIDE_THRESHOLD = 2.0
...
class Bot:
    ...
    def respond(self, obs, quote, turn: int):
        ...
        # -- every earlier turn --
        # Ride. An early fill has to beat the option we are holding, and a
        # thin one does not. SUBSTITUTE caps this round's loss at 2, which
        # makes crossing cheap, so it lowers the bar.
        bar = RIDE_THRESHOLD
