...
# -- Reading the auction tape ----------------------------------------
# obs.auction_log publishes every WIN and what was paid for it. A losing bid
# stays private, but a win is a signed statement about the winner's plan, and
# these constants price what each statement is worth.

#: Ticks knocked off our ride bar per tick of shift magnitude the OPPONENT
#: holds. A holder of TRICK_ROOM or STEALTH_ROCK profits from a forced fill,
#: so they will be stubborn; the counter is to settle before the fill they
#: are angling for, and to be readier to do so the more shift they hold.
TAPE_SHIFT_DENY = 1.0

#: Extra read-variance charged while the opponent holds SUBSTITUTE. Their
#: downside that round is capped at 2 ticks, so they can quote and cross far
#: more boldly than their hand justifies -- which makes their quote a worse
#: description of what they actually hold.
TAPE_SUB_SKEPTIC = 3.0

#: A FORESIGHT win above this price is treated as desperation: a player who
#: pays through the nose to look at someone else's coins is telling you they
#: cannot read their own. That means a flat hand, so their revealed sum is
#: nearer zero than their quote claims.
TAPE_DESPERATION_TE = 10
#: How far toward zero to pull their estimated sum when that fires.
TAPE_DESPERATION_PULL = 0.5

#: Buy a guaranteed win when it is cheap. They cannot bid more than they
#: hold, and obs.te_theirs is exact, so te_theirs+1 always wins -- worth
#: paying whenever it is below what the power is actually worth to us.
WALLET_SNIPE = True

#: How much the shade leans on how contested the auction actually is. At 0
#: the shade is the flat SHADE; at 1 it scales fully with the opponent's
#: remaining balance, so we shade hard against a full stack and barely at all
#: against an empty one.
SHADE_CONTEST = 1.0

#: TRANSFORM fires when their revealed sum is this much more decisive than
#: ours. A flat hand is the worthless one -- it says only what the prior
#: already said -- so the swap is worth making when they hold the hand that
#: carries more directional certainty.
TRANSFORM_MARGIN = 1.0

#: Read-variance added per tick that a Maker's opening centre is contradicted
#: by where that round actually settled. This is the only honesty signal that
#: survives the statelessness rule (see _distrust).
DISTRUST_GAIN = 0.8

#: Holding SUBSTITUTE, quote this many ticks tighter than the obligation
#: solve would otherwise choose. A tighter quote collects a bigger obligation
#: payout when it lands and is crossed more often when it does not -- and
#: being crossed is exactly the risk SUBSTITUTE caps.
SUB_BAIT_TIGHTEN = 2
...
class Bot:
    ...
    def reset(self, seat: int, config, seed: int) -> None:
        ...
        self.desperate = False              # tape: they overpaid for FORESIGHT
    ...
    def _refresh(self, obs) -> None:
        ...
        # The tape: every WIN and its price. A losing bid stays private, but
        # a win is a signed statement about the winner's plan.
        self.desperate = False
        for e in obs.auction_log:
            if e.get("seat") == self.seat:
                continue
            if (e.get("power") == "FORESIGHT"
                    and float(e.get("cost", 0)) >= TAPE_DESPERATION_TE):
                # They paid through the nose to look at OUR coins, which says
                # they cannot read their own. That is a flat hand.
                self.desperate = True

    def _distrust(self, obs) -> float:
        """Extra variance to charge this opponent's quotes, from behaviour.

        The tempting version of this -- score their quotes against the truth
        and learn who lies -- is IMPOSSIBLE here, and it is worth being clear
        why: S is revealed only after the deal ends, and carrying state
        between deals is not merely discouraged but enforced against (fresh
        module, fresh instance, fresh interpreter). Anything learned dies with
        the deal.

        What survives is within-deal contradiction. A Maker's opening centre
        is a claim about where it thinks the score is; the price that round
        eventually settled at is where both sides were willing to trade. A
        Maker whose claim is repeatedly far from where its own round settles
        is not describing its hand, and its next quote is worth less.
        """
        drift, n = 0.0, 0
        for c in obs.contracts:
            ...
            drift += abs(c.price - mid)
            n += 1
        if not n:
            return 0.0
        # A width of spread is normal disagreement; only charge beyond it.
        excess = max(0.0, drift / n - obs.final_cap)
        return DISTRUST_GAIN * excess
    ...
    def _their_k(self, obs):
        ...
        if self.reads:
            r0 = max(self.reads)
            noise = READ_NOISE + self._distrust(obs)
            if "SUBSTITUTE" in obs.powers_theirs:
                # Capped downside buys them the freedom to quote boldly, so
                # the quote describes their hand less well than usual.
                noise += TAPE_SUB_SKEPTIC
        ...
        for est, var in parts:
            if var <= 0.0:
                exact = est              # nothing else can improve on this
                break
        ...
        # A player who overpaid to see OUR coins cannot read its own hand,
        # and a hand you cannot read is a flat one. Pull toward zero.
        if self.desperate and not obs.foresight:
            est *= (1.0 - TAPE_DESPERATION_PULL)
    ...
    def quote(self, obs):
        ...
        w = self._best_width(obs)

        # SUBSTITUTE caps this round's CONTRACT loss at 2 ticks. It does not
        # cap the obligation, the width premium or the forcing fee -- those
        # are separate transfers and settle in full -- so the play is not a
        # wild quote, it is a TIGHT one: a tighter quote earns a larger
        # obligation payout when it lands and gets crossed more often when it
        # does not, and being crossed is precisely the risk that is capped.
        if "SUBSTITUTE" in obs.powers_mine:
            w = max(obs.final_cap, w - SUB_BAIT_TIGHTEN)
    ...
    def respond(self, obs, quote, turn: int):
        ...
        # Tape: a holder of TRICK_ROOM or STEALTH_ROCK is paid by the forced
        # fill, so they will stall to reach it. Deny it -- settle sooner the
        # more shift they hold, and hold out longer when the shift is ours.
        net_shift = self._shift(obs)
        bar -= TAPE_SHIFT_DENY * (-net_shift if net_shift < 0 else 0.0)
        bar += TAPE_SHIFT_DENY * (net_shift if net_shift > 0 else 0.0)
    ...
    def bid(self, obs, offered):
        ...
        for name in offered:
            ...
            # rate between the auction and the salvage at the end of the deal.
            fair = v / self.config.TE_SALVAGE

            # Shade by how contested the auction actually is. A first-price
            # bid should be shaded hard against a full stack, but shading
            # against an opponent who cannot afford to contest is just
            # throwing the power away to save energy worth 0.08 a point.
            contest = min(1.0, obs.te_theirs / max(1.0, float(self.config.TE_BUDGET)))
            shade = SHADE * ((1.0 - SHADE_CONTEST) + SHADE_CONTEST * contest)
            amount = int(fair * shade)

            # They cannot bid more than they hold and te_theirs is exact, so
            # te_theirs+1 is a GUARANTEED win. Buy it whenever it costs less
            # than the power is worth -- against a drained opponent that is
            # the whole slate for a point or two.
            sure = int(obs.te_theirs) + 1
            if WALLET_SNIPE and sure <= fair:
                amount = max(amount, sure)
            amount = min(amount, sure)
    ...
    def use_transform(self, obs) -> bool:
        ...
        Judging our hand ALONE is the weak version, though: the swap is a
        trade, so what matters is which of the two hands is better, not
        whether ours is bad. With an estimate of their revealed sum we can
        compare directly and fire only when theirs carries more directional
        certainty than ours.
        """
        tk = self._their_k(obs)
        if tk is None:
            return abs(obs.k_mine) <= FLAT
        return abs(tk[0]) > abs(obs.k_mine) + TRANSFORM_MARGIN
