...
class Bot:
    ...
    def quote(self, obs):
        ...
        # PARITY. S is the sum of N_COINS coins of +-1 with N_COINS even, so
        # S IS ALWAYS EVEN -- half the integer prices in any quote are values
        # the score can never take. Which half a window covers depends on
        # where its low end sits, and at an even width an even `lo` covers one
        # more reachable value than an odd one at the same width and the same
        # premium: at the round-5 floor of 2 that is 2 reachable values
        # instead of 1, i.e. double the straddle rate for free.
        #
        # game_config calls this "a parity arbitrage worth about a tick a
        # deal" and it is real: aligning EVEN measured +0.43 ticks/deal on the
        # honest panel and +0.57 on the board reconstructions, while aligning
        # ODD measured -0.47 -- a symmetric signature around the parity. A
        # control that shifts by +1 unconditionally is flat, so this is the
        # lattice and not a directional bias.
        if lo % 2:
            lo += 1
        return (lo, lo + w)
